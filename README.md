...
*   **⚛️ Core Frontend & Frameworks:** `React 19` | `TypeScript` | `JavaScript (ES6+)` | `Redux Toolkit`
*   **🎨 Styling & Build Tools:** `Tailwind CSS v4` | `SCSS` | `Vite` | `React Router`
*   **☁️ BaaS & AI Integration:** `Supabase (PostgreSQL)` | `OpenRouter API (LLM)`
...
#### 🏢 [Внутренняя платформа SanaUp](https://aveelon.com/) — Корпоративный SaaS-портал
...
* **Стек:** React, TypeScript, Tailwind CSS (v4), Vite, Supabase (PostgreSQL, RLS, RPC).
- **Решения:** 
  - Реализовал интерактивную Канбан-доску на чистом HTML5 Drag-and-Drop API с оптимистичными апдейтами интерфейса.
  - Интегрировал OpenAI API (`gpt-4o-mini`) через прокси-сервер для генерации контекстных советов и автоматического JSON-декомпозирования крупных задач.
  - Разработал систему RPG-геймификации с динамическим пересчётом уровней и баланса XP пользователя в реальном времени.
  - Настроил интерактивный Помодоро-таймер с автоматической синхронизацией времени при клике на карточки задач и функцию автоматической очистки выполненных карточек через 5 минут.
